...
(new function(window, document) {
    ...
    function on(event, node, fn, sign) {
        node.addEventListener(event, fn, sign);
    }
    function off(event, node, fn, sign) {
        node.removeEventListener(event, fn, sign);
    }
    function isEnum(o) {
        return o && typeof o == 'object' && typeof o.length == 'number' && !o.nodeName && o != window;
    }
    ...
    function pushSrc(src, s) {
        s = typeof s == 'string' ? parseString(s) : s;
        if (s) {
            var done;
            for (var i = 0; i < src.length; i++) {
                if (src[i].url == s.url) {
                    ...
                    if (s.type && !src[i].type) {
                        src[i].type = s.type;
                    }
                    if (s.name.length) {
                        for (var j = 0; j < s.name.length; j++) {
                            if (src[i].name.indexOf(s.name[j]) < 0) {
                                src[i].name.push(s.name[j]);
                            }
                        }
                    }
                    done = !0;
    ...
    function parseArguments(o) {
        ...
        if (typeof o == 'function') {
            ...
            for (i in o) {
                ...
                for (var c = 0; c < v.callback.length; c++) {
                    callback.push(v.callback[c]);
                }
            }
        }
    ...
    function isTypeJS(type) {
        return !type || type == 'js';
    }
    function tagByType(type) {
        return isTypeJS(type) ? 'script' : 'link';
    }
    function srcByType(type) {
        return isTypeJS(type) ? 'src' : 'href';
    }
    ...
    function load(inf, callback) {
        ...
        var load = 'load';
        var error = 'error';
        var js = isTypeJS(inf.type);
        var node = document.createElement(tagByType(inf.type));
        node.queue = [callback];
    ...
    function $import(src, callback) {
        ...
        var exec = function() {
            ...
            //     for (var j = 0; j < src[i].name.length; j++) {
            //         dispatch('@import:' + src[i].name[j], src[i]);
            //     }
            // }
        };
        if (!src.length) {
        ...
        for (i = 0; i < src.length; i++) {
            ...
            //     if (exists.queue.length) {
            //         exists.queue.push(calc);
            //     } else {
            //         calc();
            //     }
            // } else {
            //src[i].node = load(src[i], calc);
            load(src[i], calc);
            //}
        }
    }
