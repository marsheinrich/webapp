Date.CultureInfo = { name: 'en-US', englishName: 'English (United States)', nativeName: 'English (United States)', dayNames: ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'], abbreviatedDayNames: ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'], shortestDayNames: ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'], firstLetterDayNames: ['S', 'M', 'T', 'W', 'T', 'F', 'S'], monthNames: ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'], abbreviatedMonthNames: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'], amDesignator: 'AM', pmDesignator: 'PM', firstDayOfWeek: 0, twoDigitYearMax: 2029, dateElementOrder: 'mdy', formatPatterns: { shortDate: 'M/d/yyyy', longDate: 'dddd, MMMM dd, yyyy', shortTime: 'h:mm tt', longTime: 'h:mm:ss tt', fullDateTime: 'dddd, MMMM dd, yyyy h:mm:ss tt', sortableDateTime: 'yyyy-MM-ddTHH:mm:ss', universalSortableDateTime: 'yyyy-MM-dd HH:mm:ssZ', rfc1123: 'ddd, dd MMM yyyy HH:mm:ss GMT', monthDay: 'MMMM dd', yearMonth: 'MMMM, yyyy' }, regexPatterns: { jan: /^jan(uary)?/i, feb: /^feb(ruary)?/i, mar: /^mar(ch)?/i, apr: /^apr(il)?/i, may: /^may/i, jun: /^jun(e)?/i, jul: /^jul(y)?/i, aug: /^aug(ust)?/i, sep: /^sep(t(ember)?)?/i, oct: /^oct(ober)?/i, nov: /^nov(ember)?/i, dec: /^dec(ember)?/i, sun: /^su(n(day)?)?/i, mon: /^mo(n(day)?)?/i, tue: /^tu(e(s(day)?)?)?/i, wed: /^we(d(nesday)?)?/i, thu: /^th(u(r(s(day)?)?)?)?/i, fri: /^fr(i(day)?)?/i, sat: /^sa(t(urday)?)?/i, future: /^next/i, past: /^last|past|prev(ious)?/i, add: /^(\+|after|from)/i, subtract: /^(\-|before|ago)/i, yesterday: /^yesterday/i, today: /^t(oday)?/i, tomorrow: /^tomorrow/i, now: /^n(ow)?/i, millisecond: /^ms|milli(second)?s?/i, second: /^sec(ond)?s?/i, minute: /^min(ute)?s?/i, hour: /^h(ou)?rs?/i, week: /^w(ee)?k/i, month: /^m(o(nth)?s?)?/i, day: /^d(ays?)?/i, year: /^y((ea)?rs?)?/i, shortMeridian: /^(a|p)/i, longMeridian: /^(a\.?m?\.?|p\.?m?\.?)/i, timezone: /^((e(s|d)t|c(s|d)t|m(s|d)t|p(s|d)t)|((gmt)?\s*(\+|\-)\s*\d\d\d\d?)|gmt)/i, ordinalSuffix: /^\s*(st|nd|rd|th)/i, timeContext: /^\s*(\:|a|p)/i }, abbreviatedTimeZoneStandard: { GMT: '-000', EST: '-0400', CST: '-0500', MST: '-0600', PST: '-0700' }, abbreviatedTimeZoneDST: { GMT: '-000', EDT: '-0500', CDT: '-0600', MDT: '-0700', PDT: '-0800' } }; Date.getMonthNumberFromName = function (name) {
 ...
 var n = (dst || false) ? Date.CultureInfo.abbreviatedTimeZoneDST : Date.CultureInfo.abbreviatedTimeZoneStandard, p; for (p in n) {
 if (n[p] === offset) {
 return p; 
} 
} return null; 
 ...
 var y = this.getFullYear(), m = this.getMonth(), d = this.getDate(); var dow = firstDayOfWeek || Date.CultureInfo.firstDayOfWeek; var offset = 7 + 1 - new Date(y, 0, 1).getDay(); if (offset == 8) {
 offset = 1; 
 ...
 var here = this.getTimezoneOffset(), there = Number(s) * -6 / 10; this.addMinutes(there - here); return this; 
...
}; return format ? format.replace(/dd?d?d?|MM?M?M?|yy?y?y?|hh?|HH?|mm?|ss?|tt?|zz?z?/g, function (format1) {
 switch (format1) {
 case 'hh': result = p(self.getHours() < 13 ? self.getHours() : (self.getHours() - 12)); if (result == '00') {
 result = '12'; 
} return result; case 'h': result = p(self.getHours() < 13 ? self.getHours() : (self.getHours() - 12)); if (result == '0') {
 result = '12'; 
} return result; case 'HH': return p(self.getHours()); case 'H': return self.getHours(); case 'mm': return p(self.getMinutes()); case 'm': return self.getMinutes(); case 'ss': return p(self.getSeconds()); case 's': return self.getSeconds(); case 'yyyy': return self.getFullYear(); case 'yy': return self.getFullYear().toString().substring(2, 4); case 'dddd': return self.getDayName(); case 'ddd': return self.getDayName(true); case 'dd': return p(self.getDate()); case 'd': return self.getDate().toString(); case 'MMMM': return self.getMonthName(); case 'MMM': return self.getMonthName(true); case 'MM': return p((self.getMonth() + 1)); case 'M': return self.getMonth() + 1; case 't': return self.getHours() < 12 ? Date.CultureInfo.amDesignator.substring(0, 1) : Date.CultureInfo.pmDesignator.substring(0, 1); case 'tt': return self.getHours() < 12 ? Date.CultureInfo.amDesignator : Date.CultureInfo.pmDesignator; case 'zzz': case 'zz': case 'z': return ''; 
} 
}) : this._toString(); 
}; Date.now = function () {
 return new Date(); 
 ...
 return function () {
 if (j1.substring(j1.length - 1) != 's') {
 j1 += 's'; 
} return this['add' + j1](this._orient); 
}; 
}; var nf = function (n) {
 return function () {
...
} }; var $P = Date.Parsing; var _ = $P.Operators = { rtoken: function (r) {
 return function (s) {
 ...
 return function (s1) {
 return _.rtoken(new RegExp('^s*' + s1 + 's*'))(s1); 
}; 
}, stoken: function (s) {
 ...
 var px = arguments, _ = _; return _.each(_.optional(px)); 
...
}; Date.Grammar = {}; Date.Translator = { hour: function (s) {
 return function () {
 this.hour = Number(s); 
}; 
}, minute: function (s) {
 ...
 return function () {
 switch (s) {
 case 'yesterday': this.days = -1; break; case 'tomorrow': this.days = 1; break; case 'today': this.days = 0; break; case 'now': this.days = 0; this.now = true; break; 
} 
}; 
 ...
 var c = Date.CultureInfo.regexPatterns; var kx = keys.split(/\s+/), px = []; for (var i = 0; i < kx.length; i++) {
