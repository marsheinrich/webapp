...
(function($) {
    ...
    function Timepicker() {
        ...
        this.tpDiv = $('<div id="' + this._mainDivId +
    ...
    $.extend(Timepicker.prototype, {
        ...
        _showTimepicker: function(input) {
            ...
            if ((!inst.inline) && (typeof $.ui.position == 'object')) {
                ...
                var offset2 = inst.tpDiv.offset();
                $.timepicker._pos = [offset2.top, offset2.left];
            }
            ...
            if (showHours) {
                ...
                for (row = 1; row <= rows; row++) {
                    ...
                    if (row == amFirstRow && showPeriodLabels) {
                        html += '<tr class="ui-state-am">' +
                            '<th rowspan="' + amRows.toString() + '" class="periods ui-state-am" scope="row">' + amPmText[0] + '</th>';
                    }

                    // PM
                    if (row == pmFirstRow && showPeriodLabels) {
                        html += '<tr class="ui-state-pm">' +
                            '<th rowspan="' + pmRows.toString() + '" class="periods ui-state-pm" scope="row">' + amPmText[1] + '</th>';
                    }

                    for (col = 1; col <= hoursPerRow; col++) {
                        if (showPeriodLabels && row < pmFirstRow && hours[hourCounter] >= 12) {
                            html += this._generateHTMLHourCell(inst, undefined, showPeriod, showLeadingZero, true);
                        } else {
                            html += this._generateHTMLHourCell(inst, hours[hourCounter], showPeriod, showLeadingZero, false);
                            hourCounter++;
                        }
                    }
                    html += '</tr>';
...
*/
        ...
        _generateHTMLMinutes: function(inst) {
            ...
            for (row = 1; row <= rows; row++) {
                ...
                while (minuteCounter < row * minutesPerRow) {
                    var m2 = minutes[minuteCounter];
                    var displayText = '';
                    if (m2 !== undefined) {
                        displayText = (m2 < 10) && showMinutesLeadingZero ? '0' + m2.toString() : m2.toString();
                    }
                    html += this._generateHTMLMinuteCell(inst, m2, displayText);
                    minuteCounter++;
                }
                html += '</tr>';
            }
        ...
        _generateHTMLHourCell: function(inst, hour, showPeriod, showLeadingZero, isAm) {
            ...
            /*var amPmStyle = 11 >= hour
                ? 'style="background-color:#ffc500;color:#000000;"'
                : 'style="background-color:#2419b2;color:#ffffff;"';*/
            var amPmStyle = '';

            var amPmClass = 11 >= hour
                ? 'ui-state-am'
                : 'ui-state-pm';

            if (enabled) {
                html = '<td class="ui-timepicker-hour-cell" data-timepicker-instance-id="#' + inst.id.replace(/\\\\/g, '\\') +
                    '" data-hour="' + hour.toString() + '">' +
                    '<a class="' + amPmClass + ' ui-state-default ' +
                    (hour == inst.hours ? 'ui-state-active ' : ' ') + '" ' + amPmStyle + '>' +
                    displayHour.toString() +
                    '</a></td>';
            } else {
                html =
                    '<td>' +
                    '<span class="' + amPmClass + ' ui-state-default ui-state-disabled ' +
                    (hour == inst.hours ? ' ui-state-active ' : ' ') + '" ' + amPmStyle + '>' +
                    displayHour.toString() +
                    '</span>' +
                    '</td>';
        ...
        _generateHTMLMinuteCell: function(inst, minute, displayText) {
            ...
            if (enabled && hour !== null) {
                if (!isNaN(parseInt(maxTime.hour)) && !isNaN(parseInt(maxTime.minute)) && hour >= maxTime.hour && minute >
                    maxTime.minute) {
                    enabled = false;
                }
                if (!isNaN(parseInt(minTime.hour)) && !isNaN(parseInt(minTime.minute)) && hour <= minTime.hour && minute <
                    minTime.minute) {
                    enabled = false;
                }
            }
            ...
            if (enabled) {
                html = '<td class="ui-timepicker-minute-cell" data-timepicker-instance-id="#' + inst.id.replace(/\\\\/g,
       ...
       @param  time    String - the new time */
        ...
        _setTime: function(inst, time, noChange) {
            ...
            if (time instanceof Date) {
                ...
                var time2 = this.parseTime(inst, time);
                inst.hours = time2.hours;
                inst.minutes = time2.minutes;
            }
    ...
    $.fn.timepicker = function(options) {
        ...
        if (typeof options == 'string' && (options == 'getTime' || options == 'getTimeAsDate' || options == 'getHour' || options ==
            'getMinute')) {
